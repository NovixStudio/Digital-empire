/* ============================================================
   ...
   живой просмотр приложения, iOS 26 Liquid Glass UI
   ============================================================ */

const SAVE_KEY = 'appempire-save-v1';

/* ---------- GAME TIME SCALE ----------
   1 in-game hour = 60 real seconds → a full 24h in-game day takes
   GAME_DAY_SECONDS = 1440 real seconds (24 real minutes). Everything that
   used to assume "1 game day == 86400 real seconds" (day rollover, the
   in-game clock, daily tax/payroll/loan-interest accrual) is driven off
   this constant so the whole game's pacing scales together.
...
const GAME_DAY_SECONDS = 1440;          // real seconds per in-game day
...
const STATIC_EN = {
  ...
  'Переродиться':'Rebirth','Перейти к приложениям':'Go to sites','Переименовать проект':'Rename project',
  'Продолжить как есть':'Continue as is','Продать / закрыть приложение':'Sell / close site',
  ...
  'Свободные средства':'Available cash','Приложения':'Sites','Приложение в разработке':'Site under construction',
...
const CATEGORY_META = {
  content:  {icon:'📰', name:'Контент-приложения', desc:'Аудитория, просмотры, подписчики', nameEn:'Content apps', descEn:'Audience, views, subscribers'},
  commerce: {icon:'🛍️', name:'Шоппинг',          desc:'Покупки и маркетплейсы в приложении', nameEn:'Shopping', descEn:'Selling goods and services in-app'},
  software: {icon:'⚙️', name:'Утилиты и SaaS',    desc:'Подписки и рабочие инструменты', nameEn:'Utility & SaaS', descEn:'Subscriptions and productivity tools'},
  social:   {icon:'💬', name:'Соцсети и знакомства', desc:'Сообщества, лента, знакомства', nameEn:'Social & dating', descEn:'Communities, feeds, dating'},
  fintech:  {icon:'💳', name:'Финансы',            desc:'Платежи, кошельки, цифровые финансы', nameEn:'Finance', descEn:'Payments, wallets, digital finance'},
  ...
  offline:  {icon:'📡', name:'Устройства и сервисы', desc:'Хардвер и реальные услуги вокруг приложений', nameEn:'Devices & services', descEn:'Hardware and real-world services around apps'},
  games:    {icon:'🎮', name:'Игры',                 desc:'Мобильные игры и игровые студии', nameEn:'Games', descEn:'Mobile games and game studios'},
  hybrid:   {icon:'🧬', name:'Супер-приложения',    desc:'Уникальные приложения, созданные слиянием двух категорий', nameEn:'Super-apps', descEn:'Unique apps created by merging two categories'},
};
const CATEGORY_ORDER = ['content','commerce','software','social','fintech','offline','ai','games'];
...
const WEBSITE_TYPES = [
  {id:'blog',    name:'Заметки',              nameEn:'Notes app',             icon:'📝', baseCost:220,     baseIncome:1.6,  unlockNetWorth:0,       category:'content'},
  {id:'news',    name:'Новостное приложение', nameEn:'News app',              icon:'🗞️', baseCost:1050,    baseIncome:6,    unlockNetWorth:900,     category:'content'},
  {id:'shop',    name:'Шоппинг-приложение',   nameEn:'Shopping app',          icon:'🛍️', baseCost:1650,    baseIncome:10,   unlockNetWorth:1600,    category:'commerce'},
  {id:'saas',    name:'SaaS-приложение',      nameEn:'SaaS app',              icon:'⚙️', baseCost:12000,   baseIncome:58,   unlockNetWorth:33000,   category:'software'},
  {id:'app',     name:'Органайзер-продуктивность', nameEn:'Productivity app', icon:'🗂️', baseCost:22500,   baseIncome:95,   unlockNetWorth:59000,   category:'software'},
  {id:'forum',   name:'Комьюнити-приложение', nameEn:'Community app',         icon:'🗨️', baseCost:30000,   baseIncome:130,  unlockNetWorth:78000,   category:'social'},
  {id:'market',  name:'Маркетплейс-приложение', nameEn:'Marketplace app',     icon:'🏬', baseCost:27000,   baseIncome:120,  unlockNetWorth:72000,   category:'commerce'},
  {id:'video',   name:'Стриминговое приложение', nameEn:'Streaming app',      icon:'📹', baseCost:67500,   baseIncome:260,  unlockNetWorth:182000,  category:'content'},
  {id:'social',  name:'Приложение-соцсеть',   nameEn:'Social app',            icon:'💬', baseCost:90000,   baseIncome:340,  unlockNetWorth:234000,  category:'social'},
  {id:'devtool', name:'SDK для разработчиков', nameEn:'Dev SDK app',          icon:'🧰', baseCost:135000,  baseIncome:480,  unlockNetWorth:338000,  category:'software'},
  {id:'dating',  name:'Дейтинг-приложение',   nameEn:'Dating app',            icon:'💌', baseCost:165000,  baseIncome:560,  unlockNetWorth:416000,  category:'social'},
  {id:'crypto_exchange', name:'Крипто-кошелёк', nameEn:'Crypto wallet app',   icon:'🪙', baseCost:330000,  baseIncome:1050, unlockNetWorth:845000,  category:'fintech'},
  {id:'ai',      name:'AI-ассистент',         nameEn:'AI assistant app',      icon:'🤖', baseCost:600000,  baseIncome:1900, unlockNetWorth:1300000, category:'ai'},
  {id:'ai_agent',name:'Автономные AI-агенты', nameEn:'Autonomous AI agent app', icon:'🧠', baseCost:2250000, baseIncome:6800, unlockNetWorth:4550000, category:'ai'},
];
const OFFLINE_TYPES = [
  {id:'retail',     name:'Вендинг с приложением',     nameEn:'App-linked vending network', icon:'🏪', baseCost:5250,   baseIncome:26,   unlockNetWorth:12000,   category:'offline'},
  {id:'logistics',  name:'Доставка дронами',          nameEn:'Drone delivery',             icon:'🚁', baseCost:18000,  baseIncome:78,   unlockNetWorth:42000,  category:'offline'},
  {id:'restaurant', name:'Курьерская служба приложения', nameEn:'App courier service',      icon:'🛵', baseCost:39000,  baseIncome:150,  unlockNetWorth:91000,  category:'offline'},
  {id:'gym',        name:'Носимые фитнес-гаджеты',    nameEn:'Wearable fitness devices',   icon:'⌚', baseCost:82500,  baseIncome:290,  unlockNetWorth:208000, category:'offline'},
  {id:'bank',       name:'Необанк в приложении',      nameEn:'Neobank app',                icon:'🏦', baseCost:210000, baseIncome:700,  unlockNetWorth:546000, category:'offline'},
  {id:'realty',     name:'Умный дом — сеть устройств', nameEn:'Smart home device network', icon:'🏠', baseCost:480000, baseIncome:1450, unlockNetWorth:1105000, category:'offline'},
];
...
const HYBRID_RECIPES = [
  ...
  {aId:'blog',            bId:'social',    id:'hybrid_media',         name:'Медиа-холдинг',              nameEn:'Media Holding',             icon:'📢', baseIncome:1400, baseCost:260000, requiredTrackLevel:8, bonus:{value:0.20}, desc:'content + social → буст загрузок всем приложениям, +20% к общему доходу', descEn:'content + social → download boost for all apps, +20% total income'},
  ...
  {aId:'dating',          bId:'forum',     id:'hybrid_nextgen_social',name:'Соцсеть нового поколения',   nameEn:'Next-Gen Social App',       icon:'📲', baseIncome:2200, baseCost:520000, requiredTrackLevel:8, bonus:{value:0.20}, desc:'social + social → авто-прирост репутации, +20% к общему доходу', descEn:'social + social → auto reputation growth, +20% total income'},
...
/* ---------- GAMES — a dedicated sub-category (Phase 6) ----------
   Mobile games and game studios, distinct from the general "software"
   apps: they lean hard on in-app purchases (see MONETIZATION_FIT.games)
   and on the "design" track (core gameplay/production quality — see
   PRIORITY_TRACK.games) rather than raw infra. */
const GAME_TYPES = [
  {id:'arcade',         name:'Аркадная игра',     nameEn:'Arcade game',        icon:'🕹️', baseCost:1400,   baseIncome:7.5,  unlockNetWorth:2800,    category:'games'},
...
const ALL_BUSINESS_TYPES = [...WEBSITE_TYPES, ...OFFLINE_TYPES, ...GAME_TYPES, ...HYBRID_TYPES];

/* ---------- MONETIZATION MODEL (Phase 2 of the economy overhaul) ----------
   Chosen once, mandatorily, right before an app is created (see
   openMonetizationModal()/buySite()). Not cosmetic — how well the model
   fits the app's category (see MONETIZATION_FIT) directly scales its
   income via monetizationIncomeMult(). Apps created before this system
...
const MONETIZATION_MODELS = [
  {id:'paid_app',  name:'Платное приложение', nameEn:'Paid app',       icon:'💰', desc:'Разовая оплата за установку — простой, предсказуемый доход',                                     descEn:'One-time payment on install — simple, predictable revenue'},
  {id:'ads',       name:'Реклама',            nameEn:'Advertising',    icon:'📢', desc:'Доход от показов и кликов по рекламе — чем больше пользователей, тем больше денег',              descEn:'Revenue from ad impressions and clicks — the more users, the more money'},
  {id:'donations', name:'Донаты',             nameEn:'Donations',      icon:'🎁', desc:'Добровольные взносы пользователей — не давит на аудиторию, но заработок скромнее',                descEn:"Voluntary user donations — gentle on the audience, but modest earnings"},
  {id:'iap',       name:'Платный контент',    nameEn:'Paid content',   icon:'💎', desc:'Платный контент и подписки внутри приложения — доход растёт с вовлечённостью',                    descEn:'In-app paid content and subscriptions — revenue grows with engagement'},
];
// How well each monetization model fits each app category. >1 = boosts
// income, <1 = hurts it. Deliberately not flat: a mismatched model
// (donations on a crypto wallet, ads on a neobank) should visibly hurt.
const MONETIZATION_FIT = {
  content:  {ads:1.20, paid_app:0.90, donations:0.85, iap:1.10},
  commerce: {ads:0.80, paid_app:0.85, donations:0.60, iap:1.30},
  software: {ads:0.75, paid_app:1.25, donations:0.65, iap:1.15},
  social:   {ads:1.15, paid_app:0.80, donations:1.05, iap:1.05},
  fintech:  {ads:0.65, paid_app:1.10, donations:0.55, iap:1.25},
  ai:       {ads:0.75, paid_app:1.20, donations:0.60, iap:1.25},
  offline:  {ads:1.00, paid_app:0.95, donations:0.65, iap:1.15},
  games:    {ads:1.05, paid_app:0.85, donations:0.55, iap:1.35},
  hybrid:   {ads:1.00, paid_app:1.00, donations:1.00, iap:1.00},
};
...
/* ---------- PRIORITY TRACK (Phase 5) ----------
   Which of the three upgrade tracks matters most for a given app
   category. Shown to the player UP FRONT (a badge in the app view, next
   to the tracks) — see PRIORITY_TRACK's counterpart in script-sites.js
   for the same idea applied to websites. */
const PRIORITY_TRACK = {
  content:  'traffic',  // media apps live and die by audience size
  commerce: 'design',   // in-app storefront/UX drives conversion
  software: 'infra',    // utilities/SaaS live or die on reliability
  ...
  offline:  'design',   // companion hardware/service polish
...
// Legacy apps (created before this system) have no .monetization — keep them
...
const TRACK_META = {
  design:       {name:'UI/UX Дизайн',    nameEn:'UI/UX Design',    icon:'🎨', color:'var(--purple)', textOn:'#fff',    costMult:0.32, incomeGrowth:0.12,  desc:'Внешний вид и первое впечатление пользователей', descEn:'Look and feel, user first impressions'},
  traffic:      {name:'Загрузки',        nameEn:'Downloads',       icon:'⬇️', color:'var(--teal)',   textOn:'#04170a', costMult:0.42, incomeGrowth:0.20,  desc:'Приток новых пользователей из сторов', descEn:'Inflow of new users from app stores'},
  infra:        {name:'Серверы',         nameEn:'Servers',         icon:'☁️', color:'var(--blue)',   textOn:'#fff',    costMult:0.24, incomeGrowth:0.09,  desc:'Стабильность, скорость, бэкенд-команда', descEn:'Stability, speed, backend team'},
};
const TRACK_ORDER = ['design','traffic','infra'];
...
const AD_TYPES = [
  {id:'banner', category:'ad', icon:'🖼️', name:'Баннер в приложении', nameEn:'In-app banner', payoutSeconds:60,  penalty:0.04, durationMs:180000, minTraffic:1},
  {id:'popup',  category:'ad', icon:'🔔', name:'Interstitial-реклама', nameEn:'Interstitial ad', payoutSeconds:150, penalty:0.09, durationMs:180000, minTraffic:1},
  {id:'video',  category:'ad', icon:'🎬', name:'Rewarded-видео',      nameEn:'Rewarded video',   payoutSeconds:300, penalty:0.16, durationMs:240000, minTraffic:1},
...
const PRODUCT_TYPES = [
  {id:'prod_basic',     category:'product', icon:'📦', name:'Дешёвые IAP',         nameEn:'Cheap IAP',        costSeconds:25,  incomeBonus:0.05, durationMs:240000, minTraffic:1},
  {id:'prod_standard',  category:'product', icon:'🎁', name:'Стандартные IAP',     nameEn:'Standard IAP',     costSeconds:80,  incomeBonus:0.13, durationMs:300000, minTraffic:1},
  {id:'prod_premium',   category:'product', icon:'💎', name:'Премиум-подписка',    nameEn:'Premium subscription', costSeconds:220, incomeBonus:0.28, durationMs:360000, minTraffic:4},
  {id:'prod_exclusive', category:'product', icon:'👑', name:'VIP-подписка',        nameEn:'VIP subscription', costSeconds:600, incomeBonus:0.55, durationMs:420000, minTraffic:7},
];
...
function placeSlotItem(idx, typeId){
  ...
  if(site.tracks.traffic < slotType.minTraffic){ toast(tr('Нужен более высокий уровень загрузок','Needs a higher downloads level')); playSound('error'); return; }
    ...
    return `<div class="card glass" style="margin-bottom:10px;${(locked||affordBlock)?'opacity:.5;':''}">
      <div class="card-row">
        ...
        <div style="flex:1">
          ...
          <div class="card-sub">${locked?`${tr('нужны загрузки','needs downloads')} ${a.minTraffic}+`:effectTxt}</div>
        </div>
      </div>
...
/* ---------- SITE RENOVATION (single-site staged upgrade loop) ----------
   ...
   приложению после каждого обновления становятся дешевле (более опытная,
   оптимизированная команда), плюс приложение получает небольшой постоянный
   бонус к доходу. Не путать со слиянием (mergeSites) — это апгрейд ОДНОГО
   приложения, а не объединение двух. */
...
const RENOVATION_SALARY_DECAY = 0.88;  // зарплаты на приложении *= this за этап (дешевеют)
const RENOVATION_INCOME_BONUS = 0.10;  // + постоянный доход за этап (аддитивно к mult)
const RENOVATION_MAX_STAGE = 5;        // максимум обновлений на приложение
const IPO_MIN_RENOVATIONS = 2;         // нужно минимум столько обновлений приложения, чтобы вывести его на IPO
/* ---------- TAXES ----------
...
function renovateSite(idx){
  ...
  if(!site) return;
  const type = ALL_BUSINESS_TYPES.find(t=>t.id===site.typeId);
  if(renovationStage(site) >= RENOVATION_MAX_STAGE){ toast(tr('Достигнут максимум обновлений для этого приложения','This site has reached its max renovations')); playSound('error'); return; }
  ...
  toast(`🛠️ ${tr('Приложение обновлено!','Site renovated!')} +${RENOVATION_CAP_BONUS} ${tr('к потолку уровня','to the level cap')}`);
...
/* ---------- FIRST-SESSION PACING ----------
   ...
   tax/payroll bills start being assessed. All real-time (not game-time)
   so it behaves the same regardless of difficulty. */
...
const CEO_SKILLS = [
  ...
  {id:'cheap_merge',  name:'M&A отдел',                icon:'🤝', cost:6, desc:'Слияние приложений дешевле на 30% навсегда'},
  {id:'extra_slot',   name:'Расширение штаб-квартиры', icon:'🏢', cost:8, desc:'+1 слот под приложения навсегда'},
];
...
const ACHIEVEMENTS = [
  {id:'first_site',    icon:'🌱', name:'Первый бизнес',       nameEn:'First business',        desc:'Установите своё первое приложение',                descEn:'Buy your first site',                reward:500,    cond:s=>s.sites.length>=1},
  {id:'five_sites',     icon:'🏗️', name:'Растущая империя',    nameEn:'Growing empire',        desc:'Владейте 5 приложениями одновременно',        descEn:'Own 5 sites at the same time',        reward:5000,   cond:s=>s.sites.length>=5},
  {id:'ten_sites',      icon:'🏙️', name:'Конгломерат',         nameEn:'Conglomerate',          desc:'Владейте 10 приложениями одновременно',       descEn:'Own 10 sites at the same time',       reward:25000,  cond:s=>s.sites.length>=10},
  ...
  {id:'first_merge',    icon:'🔗', name:'Синергия',            nameEn:'Synergy',               desc:'Слейте два приложения одного типа',           descEn:'Merge two sites of the same type',    reward:10000,  cond:s=>s.sites.some(site=>(site.merged||0)>0)},
  ...
  {id:'minimalist',     icon:'🧘', name:'Минималист',          nameEn:'Minimalist',            desc:'Достигните $1 000 000 чистых активов, владея не более чем 3 приложениями', descEn:'Reach $1,000,000 net worth while owning at most 3 sites', reward:30000, cond:s=>netWorth()>=1e6 && s.sites.length<=3, hidden:true, skillPoints:1},
  {id:'collector',      icon:'🗃️', name:'Коллекционер',        nameEn:'Collector',             desc:'Владейте хотя бы одним приложениеом каждой категории', descEn:'Own at least one site in every category', reward:25000, cond:s=>CATEGORY_ORDER.every(cat=>s.sites.some(site=>{const t=ALL_BUSINESS_TYPES.find(tt=>tt.id===site.typeId); return t&&t.category===cat;})), hidden:true},
];
...
/* ============================================================
   ...
   ============================================================ */
const QUEST_POOL = [
  {key:'buy_site',        icon:'🌐', name:'Установите новое приложение',                nameEn:'Buy a new site',                  target:1, reward:0.6},
  {key:'upgrade_track',   icon:'📐', name:'Прокачайте треки приложений 3 раза',   nameEn:'Upgrade site tracks 3 times',     target:3, reward:0.5},
  ...
  {key:'open_site',       icon:'🖥️', name:'Откройте карточку любого приложения',  nameEn:'Open any site card',              target:1, reward:0.2},
...
function buildStatsHtml(){
  ...
  return `<h3>📊 ${tr('Общая статистика','Overall stats')}</h3>
    ...
    <div class="stat-strip">
      ...
      <div class="stat-box glass"><div class="lbl">${tr('Лучшее приложение','Best site')}</div><div class="val" style="font-size:13px;">${esc(bestSiteName())}</div></div>
    </div>
    <div class="btn-row"><button class="btn btn-outline btn-block" onclick="closeModal()">${S('Закрыть')}</button></div>`;
}
...
/* ============================================================
   ...
   ============================================================ */
const SEASON_THEMES = [
  {name:'Неделя роста загрузок',    nameEn:'Download Growth Week',  icon:'📈', desc:'Заработайте суммарно {X} за эту неделю', descEn:'Earn a total of {X} this week', mult:400},
  ...
  {name:'Неделя экспансии',        nameEn:'Expansion Week',       icon:'🏗️', desc:'Владейте {X} приложениями одновременно',        descEn:'Own {X} sites at the same time',      mult:1, isCount:true, count:6},
];
...
function openRebirthModal(){
  ...
  openModal(
    '<h3>🔄 Переродиться?</h3>' +
    '<p style="color:var(--dim);font-size:13px;margin-bottom:10px;">Наличные, приложения, акции и большая часть активов будут потеряны. Взамен вы получите очки наследия и постоянный множитель дохода.</p>' +
    finalWarning +
...
function runAutoUpgrade(){
  ...
  state.sites.forEach((site,idx)=>{
    if(site.dev) return; // already mid-development — one cycle at a time
    if(site.bugged){ if(state.cash > bugPatchCost(site)*5) startPatch(idx); return; }
    const type = ALL_BUSINESS_TYPES.find(t=>t.id===site.typeId);
    ...
    if(cheapest && state.cash > cheapestCost*5) startFeatureDev(idx, cheapest, 1);
  });
}
...
function unlockAutoManager(idx){
  ...
  log(`🤖 Разблокирован Авто-менеджер приложений за ${fmt(cost)} — теперь доступен на каждом приложении`);
...
function triggerInfraDowntime(){
  ...
  log('⚠️ Нехватка инфраструктуры вызвала сбой на «'+site.name+'» — доход −50%. Прокачайте инфраструктуру или застрахуйте приложение, чтобы это не повторялось.');
  renderEvents();
}
...
function triggerViral(){
  ...
  log('🔥 Вирусный тренд подхватил категорию «'+label+'» — доход этих приложений +60% на 90с');
  renderEvents();
}
...
function buildTaxHtml(){
  ...
  if(!cats.length){
    return `<p style="color:var(--dim);font-size:13px;">${tr('Пока нет приложений, приносящих налогооблагаемый доход.','No sites generating taxable income yet.')}</p>`;
  }
  const total = totalTaxOwed();
  ...
  return `<p style="color:var(--dim);font-size:12.5px;margin-bottom:12px;">${tr(`Каждый игровой день с каждой категории приложений начисляется налог ${Math.round(state.taxes.rate*100)}% от дневного дохода. Если не платить ${TAX_AUDIT_DAYS}+ дней подряд, категорию начинают проверять — доход падает, пока долг не закрыт.`,`Every in-game day, each category of sites accrues a ${Math.round(state.taxes.rate*100)}% tax on its daily income. Leave it unpaid ${TAX_AUDIT_DAYS}+ days in a row and the category gets audited — income drops until the debt is cleared.`)}</p>
...
/* ---------- PAYROLL (Phase 1 of the economy overhaul) ----------
   Every 30 in-game days, all hired staff (across every app) are due their
   ...
   game over. Apps mode has its own independent copy of this system. */
const PAYROLL_PERIOD_DAYS = 30;
...
function assessPayroll(){
  ...
  log(`💸 ${tr('Начислена зарплата команде','Team payroll due')}: ${fmt(due)}`);
...
function assessPayrollOverdue(){
  ...
  if(state.payroll.overdueDays >= PAYROLL_AUDIT_DAYS && !state.payroll.audited){
    ...
    toast(`💸 ${tr('Команда недовольна задержкой зарплаты!','The team is unhappy about the pay delay!')}`);
...
function buildPayrollHtml(){
  ...
  return `<p style="color:var(--dim);font-size:12.5px;margin-bottom:12px;">${tr(`Раз в ${PAYROLL_PERIOD_DAYS} игровых дней вся нанятая команда (сейчас ${staffCount} чел.) получает зарплату — сейчас это ${fmt(monthly)}/мес суммарно. Если не платить ${PAYROLL_AUDIT_DAYS}+ дней подряд, доход падает на 25%, пока долг не закрыт.`,`Every ${PAYROLL_PERIOD_DAYS} in-game days, the whole hired team (currently ${staffCount}) is due salary — right now that's ${fmt(monthly)}/mo total. Leave it unpaid ${PAYROLL_AUDIT_DAYS}+ days in a row and income drops 25% until it's cleared.`)}</p>
...
function openPayrollModal(){ openModal(`<h3>💸 ${tr('Зарплата команды','Team payroll')}</h3><div id="payroll-modal-body">${buildPayrollHtml()}</div>`); }
...
/* ---------- INCOME / EXPENSES DETAIL (Phase 1) ----------
   ...
   finance card — every category of app income listed separately, and
...
function expenseBreakdown(){
  ...
  if(monthlyPayroll>0) rows.push({icon:'💸', label:tr('Зарплата команды','Team payroll'), perSec: monthlyPayroll/(PAYROLL_PERIOD_DAYS*GAME_DAY_SECONDS)});
  if(state.taxes){
...
function buildFinanceDetailHtml(){
  ...
  return `
    ...
    ${rowsHtml(inc, tr('Пока нет приложений, приносящих доход.','No apps generating income yet.'))}
...
// of which app they work at. That made hiring punishingly expensive
// relative to a cheap early app (a $2100 hire on a $220 notes app) while
// being trivially cheap relative to a late-game app (the same $2100 on a
// $2.25M AI-agent app). Both now scale with the app's own
// baseCost/baseIncome, the same way hosting-equivalent costs already did
// elsewhere — so hiring stays proportionate at every tier.
const EMPLOYEE_HIRE_COST_SHARE = 0.9;   // hire cost ≈ this × the app's own baseCost, before the per-hire 1.35^n growth
const EMPLOYEE_SALARY_SHARE = 0.012;    // salary per pay period ≈ this × one period's worth of the app's raw baseIncome
...
// hire AND draws a real monthly salary — see assessPayroll(). Apps mode has
// its own independent copy of this system (not shared with Sites mode).
const EMPLOYEE_LEVELS = [
  {level:1, name:'Джуниор',    nameEn:'Junior',      icon:'🌱', statMult:1.00, salaryMult:1.0},
  {level:2, name:'Мидл',       nameEn:'Middle',      icon:'💼', statMult:1.35, salaryMult:1.9},
  ...
  {level:5, name:'CTO',        nameEn:'CTO',         icon:'👑', statMult:3.00, salaryMult:8.0},
];
function empLevelMeta(level){ return EMPLOYEE_LEVELS[Math.min(EMPLOYEE_LEVELS.length, Math.max(1,level))-1]; }
// Monthly salary for one employee of a given level, on a given business
// type (paid every 30 in-game days — see assessPayroll()). Scales with the
// app's own baseIncome so payroll stays a meaningful, felt cost at every
...
// One-time hiring cost, scaled by the app's own baseCost (so hiring is
// proportionate whether it's a $220 notes app or a $2.25M AI-agent app),
...
function employeeHireCost(site, level){
  ...
  const base = Math.max(EMPLOYEE_BASE_COST*0.3, (type?type.baseCost:EMPLOYEE_BASE_COST) * EMPLOYEE_HIRE_COST_SHARE);
  return Math.round(base * Math.pow(1.35, site.employees) * empLevelMeta(level).salaryMult * difficultyCostMult() * (hasSkill('cheap_hire')?0.85:1) * renoSalary);
}
...
function staffStatBonus(site){
  ...
  return site.staffLevels.reduce((sum,lv)=>sum + empLevelMeta(lv).statMult*EMPLOYEE_INCOME_BONUS, 0);
}
...
const STAGE_META = {
  ...
  3:{label:'Растущие загрузки',       icon:'📈'},
...
function defaultState(){
  return {
    ...
    billGraceUntilDay: 0,       // taxes/payroll stay quiet until state.day passes this — eases the first session in
    log: [],
    lastSeen: Date.now(),
    dailyStreak: {count:0, lastClaim:null},
    achievements: {},
    loan: {principal:0, rating:0, takenDay:null, type:null, dueDay:null, lumpTotal:0},  // rating grows with on-time full repayments — see loanRate()/maxLoanAmount()
...
function migrate(){
  ...
  // Manual ×2/×4 acceleration was removed (Phase 4) — game always runs at ×1 now.
  ...
  state.sites.forEach(s=>{ if(typeof s.analyst !== 'boolean') s.analyst = false; });
  state.sites.forEach(s=>{ if(typeof s.dev === 'undefined') s.dev = null; if(typeof s.bugged === 'undefined') s.bugged = null; });
  ...
  state.sites.forEach(s=>{
    ...
    if(!s.tracks){
      s.tracks = {design:1,traffic:1,infra:1};
      delete s.level;
    }
...
/* ---------- FEATURE DEVELOPMENT PIPELINE (Phase 5) ----------
   The key structural difference from websites (see script-sites.js, where
   a track upgrade applies instantly): an app's staff fill a progress bar
   for a feature before it ships. Flow:
     1) startFeatureDev()  — pay upfront, opens a dev cycle (one at a time
        per app — see site.dev)
     2) tickAppDev()       — called every tick; staff headcount fills the
        progress bar over real time
     3) releaseFeature()   — on completion, the track level actually goes
        up, fired as a distinct "release" event, then review rolls for bugs
     4) if buggy: startPatch() opens a *second kind* of dev cycle — a
        patch — which also takes real dev time (unlike a website patch,
        which is an instant pay-to-fix — see script-sites.js) */
const DEV_SECONDS_PER_LEVEL = 70;   // base dev time for 1 track level with a skeleton crew
const DEV_EMPLOYEE_SPEEDUP = 0.10;  // each employee shaves this fraction off the remaining time (diminishing)
const PATCH_SECONDS = 45;           // fixed dev time to ship a patch, regardless of severity
const DEV_SPEED_MULTIPLIER = 4; // global speed-up factor for update/patch development
function devSpeedMult(site){
  return (1 + site.employees * DEV_EMPLOYEE_SPEEDUP) * DEV_SPEED_MULTIPLIER;
}
function startFeatureDev(idx, trackId, qty=1){
  const site = state.sites[idx];
  const type = ALL_BUSINESS_TYPES.find(t=>t.id===site.typeId);
  if(site.dev){ toast(tr('Уже идёт разработка — дождитесь релиза','A feature is already in development — wait for the release')); return; }
  const curLevel = site.tracks[trackId];
  const capLevel = trackMaxLevel(site);
  if(curLevel >= capLevel){ toast(tr('Макс. уровень — объедините приложение или переродитесь','Max level — merge the app or rebirth')); playSound('error'); return; }
  ...
  if(state.cash < cost){ toast(tr('Недостаточно средств','Not enough cash')); playSound('error'); return; }
  state.cash -= cost;
  site.dev = {kind:'feature', trackId, qty:n, need: DEV_SECONDS_PER_LEVEL*n, progress:0, cost};
  const meta = TRACK_META[trackId];
  log(`🛠️ ${esc(site.name)}: ${tr('начата разработка','feature dev started')} — ${L(meta,'name')} → ${tr('ур.','lvl')} ${curLevel+n}`);
  toast(`🛠️ ${tr('В разработке','In development')}: ${L(meta,'name')} → ${tr('ур.','lvl')} ${curLevel+n}`);
  playSound('buy');
  renderAll(); save();
  refreshSiteViewSections(idx, ['tracks']);
}
function startPatch(idx){
  const site = state.sites[idx];
  ...
  if(state.cash < cost){ toast(tr('Недостаточно средств','Not enough cash')); playSound('error'); return; }
  state.cash -= cost;
  site.dev = {kind:'patch', need: PATCH_SECONDS, progress:0, cost};
  log(`🔧 ${esc(site.name)}: ${tr('начата разработка патча','patch dev started')}`);
  toast(`🔧 ${tr('Патч в разработке','Patch in development')}`);
  playSound('buy');
  renderAll(); save();
  refreshSiteViewSections(idx, ['tracks']);
}
// Called once per tick for every app with an active dev cycle (see tick()).
function tickAppDev(site, idx){
  if(!site.dev) return;
  site.dev.progress += devSpeedMult(site);
  if(site.dev.progress >= site.dev.need){
    if(site.dev.kind==='patch') finishPatch(site, idx);
    else releaseFeature(site, idx);
  }
}
function releaseFeature(site, idx){
  const type = ALL_BUSINESS_TYPES.find(t=>t.id===site.typeId);
  const trackId = site.dev.trackId, qty = site.dev.qty;
  site.tracks[trackId] += qty;
  site.dev = null;
  site.lastUpgradeAt = Date.now();
  bumpQuest('upgrade_track', qty);
  const meta = TRACK_META[trackId];
  log(`🚀 ${tr('Релиз обновления','Update released')}: «${esc(site.name)}» — ${L(meta,'name')} → ${tr('ур.','lvl')} ${site.tracks[trackId]}`);
  toast(`🚀 ${tr('Релиз','Released')}: ${L(meta,'name')} → ${tr('ур.','lvl')} ${site.tracks[trackId]}`);
  playSound('upgrade');
  rollForBug(site, trackId); // reviews come in after the release — some of them uncover bugs
  renderAll(); save();
...
/* ---------- BUGS (Phase 5) ----------
   Shipping a release has a real chance of surfacing a bug in reviews —
   see rollForBug(), called from releaseFeature() above. Fixing one means
   opening a *patch* dev cycle (startPatch()), which — unlike a website's
   instant pay-to-fix — takes real development time (PATCH_SECONDS). */
...
function rollForBug(site, trackId){
  ...
  const chance = BUG_CHANCE_BY_TRACK[trackId] || 0.06;
  if(Math.random() < chance){
    const severity = Math.random() < 0.3 ? 'major' : 'minor';
    site.bugged = {severity, foundDay: state.day};
    log(`🐞 ${tr('Отзывы вскрыли баг в','Reviews uncovered a bug in')} «${esc(site.name)}» (${severity==='major'?tr('серьёзный','major'):tr('мелкий','minor')})`);
    toast(`🐞 ${tr('Баг в','Bug in')} «${esc(site.name)}» — ${tr('нужен патч','a patch is needed')}`);
  }
}
...
function siteIncome(type, site){
  ...
  return type.baseIncome * trackIncomeMultiplier(site) * empBonus * aiBonus * boostBonus * eventBonus * synergyMult * ipoMult * renoMult * adMult * productMult * payrollMult * monMult * bankruptcyMult * bugMult * contentMult * platformMult * starterMult;
}
...
function tick(){
  ...
  if(state.secondsElapsed >= GAME_DAY_SECONDS){
    ...
    if(activeScreen==='dash'){ renderFinanceCard(); renderTaxCard(); renderPayrollCard(); }
    refreshTaxModal();
    refreshPayrollModal();
  ...
  runAutoUpgrade();
  runSiteAutoManagers();
  state.sites.forEach((site,idx)=>tickAppDev(site, idx));
...
// now requires a monetizationId and won't create an app without one.
function openMonetizationModal(typeId){
  ...
  const rows = MONETIZATION_MODELS.map(function(m){
    const fit = monetizationFitMult(type, m.id);
    const lbl = monetizationFitLabel(fit);
    return `<div class="card glass" style="margin-bottom:10px;">
      <div class="card-row">
        ...
        <div style="flex:1">
          ...
          <div class="card-sub">${lbl.icon} ${lbl.text} · ×${fit.toFixed(2)} ${tr('к доходу','to income')}</div>
        </div>
      </div>
  ...
  openModal(`<h3>${type.icon} ${tr('Модель монетизации','Monetization model')}</h3>
    <p style="color:var(--dim);font-size:12.5px;margin-bottom:14px;">${tr(`Прежде чем выпустить «${L(type,'name')}», выберите, как приложение будет зарабатывать. От того, насколько модель подходит тематике, зависит прибыльность.`,`Before launching "${L(type,'name')}", choose how the app will make money. Profitability depends on how well the model fits the theme.`)}</p>
    ${rows}
...
function buySite(typeId, monetizationId){
  ...
  if(nw < type.unlockNetWorth && owned===0){ toast(tr('Ещё не открыто','Not unlocked yet')); playSound('error'); closeModal(); return; }
  if(state.sites.length >= maxSiteSlots(nw)){ toast(tr('Нет свободных слотов — растите активы','No free slots — grow your net worth')); playSound('error'); closeModal(); return; }
  if(state.sites.length>0 && !allOwnedSitesFullyRenovated()){
    toast(tr(`Сначала доведите текущие приложения до макс. обновления (эт. ${RENOVATION_MAX_STAGE}/${RENOVATION_MAX_STAGE})`, `First take your current sites to max renovation (stage ${RENOVATION_MAX_STAGE}/${RENOVATION_MAX_STAGE})`));
    playSound('error'); closeModal(); return;
  }
  ...
  if(state.cash < cost){ toast(tr('Недостаточно средств','Not enough cash')); playSound('error'); closeModal(); return; }
  state.cash -= cost;
  state.sites.push({typeId, uid:genUid(), name:pickSiteName(typeId), employees:0, tracks:{design:1,traffic:1,infra:1}, ads:[], aiModel:{kind:null, ownLevel:0}, boostUntil:0, insured:false, analyst:false, autoManager:false, ipoed:false, dev:null, bugged:null, content:[], platforms:[], incomeHistory:[], lastUpgradeAt:Date.now(), renovationStage:0, renovationSalaryMult:1, renovationIncomeMult:1, monetization:monetizationId});
  bumpQuest('buy_site');
...
function openMergeModal(idx){
  ...
  if(!cands.length){ toast(tr('Нужно второе приложение того же типа на макс. уровне трека','Need a second site of the same type at max track level')); return; }
  ...
  openModal(`<h3>🔗 ${tr('Объединить','Merge')} «${esc(a.name)}»</h3>
    <p style="color:var(--dim);font-size:12.5px;margin-bottom:14px;">${tr(`Второе приложение исчезнет, слот освободится. Объединённое приложение получит +${MERGE_CAP_BONUS} к потолку уровня трека и уйдёт в простой на ${Math.round(MERGE_DOWNTIME_MS/1000)}с без дохода.`,`The second site will disappear, freeing up a slot. The merged site gets +${MERGE_CAP_BONUS} to its track level cap and goes idle for ${Math.round(MERGE_DOWNTIME_MS/1000)}s with no income.`)}</p>
    ${rows}
    <div class="btn-row"><button class="btn btn-outline btn-block" onclick="closeModal()">${S('Отмена')}</button></div>`);
}
...
/* ---------- OWN CONTENT (Phase 6) ----------
   A small catalog of self-authored content add-ons an app can buy into,
   independent of the track/employee/monetization systems — the roadmap's
   "самостоятельное наполнение". Each is a one-time purchase per app that
   adds a permanent, stacking income bonus. Every item fits every
   category, but items whose tag matches the app's category are flagged
   as an especially good fit (a small nudge, not a hard requirement). */
const CONTENT_ITEMS = [
  {id:'blog',    icon:'📝', name:'Блог',            nameEn:'Blog',           costMult:0.15, bonus:0.06, fits:['content','hybrid']},
  {id:'forum',   icon:'💬', name:'Форум',           nameEn:'Forum',          costMult:0.18, bonus:0.07, fits:['social','content']},
  {id:'shop',    icon:'🛍️', name:'Магазин',         nameEn:'Shop',           costMult:0.25, bonus:0.09, fits:['commerce']},
  {id:'podcast', icon:'🎙️', name:'Подкаст',         nameEn:'Podcast',        costMult:0.20, bonus:0.07, fits:['content','social']},
  {id:'api',     icon:'🔌', name:'Публичное API',   nameEn:'Public API',     costMult:0.30, bonus:0.10, fits:['software','ai','fintech']},
  {id:'events',  icon:'🎉', name:'Ивенты',          nameEn:'Events',         costMult:0.20, bonus:0.08, fits:['social','offline']},
];
function contentItemCost(site, item){
  const type = ALL_BUSINESS_TYPES.find(t=>t.id===site.typeId);
  return Math.round(type.baseCost * item.costMult * difficultyCostMult());
}
...
function buyContentItem(idx, itemId){
  ...
  playSound('buy');
  renderAll(); save();
  refreshSiteViewSections(idx, ['content']);
}
function buildContentHtml(idx){
  ...
  if(!site.content) site.content = [];
  const type = ALL_BUSINESS_TYPES.find(t=>t.id===site.typeId);
  let html = `<div class="section-title">🧩 ${tr('Собственный контент','Own content')}</div>`;
    ...
    return `<div class="card glass" style="margin-bottom:8px;${owned?'border-color:rgba(48,209,88,.35);':''}">
      <div class="card-row">
        ...
        ${owned
          ...
          : `<button class="btn btn-outline" ${state.cash<cost?'disabled':''} onclick="buyContentItem(${idx},'${item.id}')">${fmt(cost)}</button>`}
      </div>
    </div>`;
  }).join('');
  return html;
...
/* ---------- PLATFORMS / PUBLISHING (Phase 6) ----------
   Publishing an app to more stores/devices than just its home platform —
   each is a one-time unlock per app with a permanent, stacking income
   bonus (wider reach = more installs). Same shape as CONTENT_ITEMS but a
   conceptually separate catalog, since the roadmap calls out multi-
   platform publishing as its own feature. */
const PLATFORM_ITEMS = [
  {id:'ios',     icon:'🍎', name:'iOS',                nameEn:'iOS',              costMult:0.15, bonus:0.09},
  {id:'android', icon:'🤖', name:'Android',            nameEn:'Android',          costMult:0.12, bonus:0.09},
  {id:'web',     icon:'🌐', name:'Web-версия',         nameEn:'Web version',      costMult:0.10, bonus:0.06},
  {id:'desktop', icon:'🖥️', name:'Десктоп',            nameEn:'Desktop',          costMult:0.18, bonus:0.06},
];
...
function buyPlatform(idx, itemId){
  ...
  playSound('buy');
  renderAll(); save();
  refreshSiteViewSections(idx, ['platforms']);
}
function buildPlatformsHtml(idx){
  const site = state.sites[idx];
  if(!site) return '';
  if(!site.platforms) site.platforms = [];
  let html = `<div class="section-title">📡 ${tr('Публикация','Publishing')}</div>`;
    ...
    return `<div class="card glass" style="margin-bottom:8px;${owned?'border-color:rgba(48,209,88,.35);':''}">
      <div class="card-row">
        ...
        ${owned
          ...
          : `<button class="btn btn-outline" ${state.cash<cost?'disabled':''} onclick="buyPlatform(${idx},'${item.id}')">${fmt(cost)}</button>`}
      </div>
    </div>`;
  }).join('');
  return html;
...
function buyInsurance(idx){
  ...
  state.cash -= cost;
  site.insured = true;
  log(`🛡️ Приложение «${esc(site.name)}» защищено от атак и сбоев инфраструктуры`);
  toast(tr('Приложение застраховано','Site insured'));
  playSound('buy');
  renderAll(); save();
  refreshSiteViewSections(idx, ['insurance']);
}
/* ---------- ANALYST (Phase 4) ----------
   A one-time hire per app, separate from the regular staff headcount.
   Doesn't affect income directly — instead it unlocks a concrete success
   probability readout, computed from real factors already tracked
   elsewhere (monetization fit, track balance, staffing, overdue bills,
   debt load) instead of a vague hint. Per the roadmap: hints are shown
   up front, not as an after-the-fact penalty. */
function analystCost(site){
  const type = ALL_BUSINESS_TYPES.find(t=>t.id===site.typeId);
  return Math.round(type.baseCost * 0.5 * difficultyCostMult());
}
function hireAnalyst(idx){
  const site = state.sites[idx];
  if(!site || site.analyst){ return; }
  const cost = analystCost(site);
  if(state.cash < cost){ toast(tr('Недостаточно средств','Not enough cash')); playSound('error'); return; }
  state.cash -= cost;
  site.analyst = true;
  log(`📊 ${tr('Нанят аналитик для','Analyst hired for')} «${esc(site.name)}»`);
  toast(tr('Аналитик нанят','Analyst hired'));
  playSound('buy');
  renderAll(); save();
  refreshSiteViewSections(idx, ['analyst']);
}
// Heuristic 0-100 "chance this app turns a real, sustained profit" score —
// deliberately not a precise simulation (see roadmap: numbers should feel
// right "by eye", not model the economy exactly). Combines the levers the
// player actually controls: how well monetization fits the category, how
// balanced the upgrade tracks are, staffing level, and any debts left
// unpaid long enough to start hurting income.
function successProbability(site){
  const type = ALL_BUSINESS_TYPES.find(t=>t.id===site.typeId);
  if(!type) return 50;
  let score = 50;
  // Monetization fit — the single biggest lever, matches the Phase 2 system.
  const fit = site.monetization ? monetizationFitMult(type, site.monetization) : 1;
  score += (fit - 1) * 100;
  // Track balance — an app that dumped everything into one track while
  // ignoring the others tends to be lopsided and fragile.
  const tv = [site.tracks.design, site.tracks.traffic, site.tracks.infra];
  const minT = Math.min(...tv), maxT = Math.max(...tv);
  const balance = maxT>0 ? minT/maxT : 1;
  score += (balance - 0.5) * 24;
  // Staffing — understaffed relative to what infra allows costs confidence.
  const cap = employeeCap(site);
  const staffRatio = cap>0 ? Math.min(1, site.employees/cap) : 1;
  score += (staffRatio - 0.4) * 15;
  // Overdue bills already flagged elsewhere as "audited" — each one is a
  // real, current drag on income, so it drags the forecast down too.
  if(state.payroll && state.payroll.audited) score -= 10;
  if(state.hosting && state.hosting.audited) score -= 10;
  if(state.taxes && state.taxes.audited){
    const cat = type.category;
    if(state.taxes.audited[cat]) score -= 8;
  }
  // An app portfolio actively bleeding cash right now (negative overall
  // balance) is a bad sign for near-term survival regardless of its own numbers.
  if(state.cash < 0) score -= 12;
  if(site.insured) score += 3;
  // Neglecting the category's priority track (see PRIORITY_TRACK) badly
  // caps upside regardless of how good everything else looks.
  const prio = priorityTrackId(type);
  const prioLvl = site.tracks[prio];
  const otherAvg = TRACK_ORDER.filter(k=>k!==prio).reduce((s,k)=>s+site.tracks[k],0) / (TRACK_ORDER.length-1);
  if(otherAvg>0 && prioLvl < otherAvg*0.6) score -= 18;
  if(site.bugged) score -= (site.bugged.severity==='major' ? 20 : 10);
  return Math.max(3, Math.min(97, Math.round(score)));
}
function successProbabilityLabel(p){
  if(p>=70) return {icon:'🟢', text: tr('высокие шансы','strong odds')};
  if(p>=40) return {icon:'🟡', text: tr('неопределённо','uncertain')};
  return {icon:'🔴', text: tr('низкие шансы','poor odds')};
}
function buildAnalystHtml(idx){
  const site = state.sites[idx];
  if(!site) return '';
  if(!site.analyst){
    const cost = analystCost(site);
    return `
    <div class="section-title">📊 ${tr('Аналитик','Analyst')}</div>
    <div class="card glass">
      <div class="card-title">${tr('Нанять аналитика','Hire an analyst')}</div>
      <div class="card-sub">${tr('Разовый платёж — аналитик рассчитывает и показывает шанс, что приложение выйдет в устойчивый плюс','One-time payment — the analyst calculates and shows the odds this app reaches a sustained profit')}</div>
      <div class="btn-row"><button class="btn btn-outline btn-block" ${state.cash<cost?'disabled':''} onclick="hireAnalyst(${idx})">${tr('Нанять за','Hire for')} ${fmt(cost)}</button></div>
    </div>`;
  }
  const p = successProbability(site);
  const lbl = successProbabilityLabel(p);
  const fit = site.monetization ? monetizationFitMult(ALL_BUSINESS_TYPES.find(t=>t.id===site.typeId), site.monetization) : 1;
  const fitLbl = monetizationFitLabel(fit);
  const tv = [site.tracks.design, site.tracks.traffic, site.tracks.infra];
  const balanced = Math.min(...tv)/Math.max(...tv) >= 0.7;
  return `
    <div class="section-title">📊 ${tr('Аналитик','Analyst')}</div>
    <div class="card glass">
      <div class="card-row">
        <div class="card-icon">${lbl.icon}</div>
        <div style="flex:1">
          <div class="card-title">${p}% — ${lbl.text}</div>
          <div class="card-sub">${tr('Шанс, что приложение выйдет в устойчивый плюс','Odds this app reaches a sustained profit')}</div>
        </div>
      </div>
      <div class="progress-bar" style="margin-top:8px;"><div style="width:${p}%;background:${p>=70?'var(--green)':p>=40?'var(--orange)':'var(--red)'};"></div></div>
      <ul style="margin:10px 0 0 18px;color:var(--dim);font-size:12px;line-height:1.7;">
        <li>${fitLbl.icon} ${tr('Монетизация','Monetization')}: ${fitLbl.text}</li>
        <li>${balanced?'👍':'⚠️'} ${tr('Ветки прокачки','Upgrade tracks')}: ${balanced?tr('сбалансированы','balanced'):tr('перекос — часть отстаёт','lopsided — one is lagging')}</li>
        <li>🎯 ${tr('Приоритет','Priority')}: ${priorityTrackLabel(ALL_BUSINESS_TYPES.find(t=>t.id===site.typeId))}</li>
        <li>${site.employees>=employeeCap(site)*0.7?'👍':'⚠️'} ${tr('Штат','Staffing')}: ${site.employees}/${employeeCap(site)}</li>
        ${site.bugged?`<li>🐞 ${tr('Активный баг','Active bug')}: ${site.bugged.severity==='major'?tr('серьёзный','major'):tr('мелкий','minor')}</li>`:''}
      </ul>
    </div>`;
}

/* ---------- INVESTORS (Phase 6) ----------
   Two ways to cash out an app to outside investors, distinct from IPO
   (which keeps the app at half income forever). Per the roadmap's
   clarification: a simple fixed revenue-share percentage, and at most
   one active deal at a time across the whole portfolio.
     • Sell forever  — one lump sum, the app leaves the portfolio for good.
     • Investor deal — a smaller cash injection now; the investor takes a
       fixed cut of that app's income automatically every tick for a
       fixed term, then the deal ends on its own and the app is fully
       yours again. */
const INVESTOR_SELL_DAYS = 12;   // sell-forever payout = this many days of the app's current income
...
function investorSellForeverAmount(idx){
  const site = state.sites[idx];
  const type = ALL_BUSINESS_TYPES.find(t=>t.id===site.typeId);
  return Math.max(200, Math.round(siteIncome(type, site) * GAME_DAY_SECONDS * INVESTOR_SELL_DAYS));
}
...
function sellToInvestorForever(idx){
  const site = state.sites[idx];
  if(!site) return;
  if(state.investorDeal && state.investorDeal.siteUid===site.uid){ toast(tr('Сначала завершите текущую сделку по этому приложению','Finish the current deal on this app first')); return; }
  const amount = investorSellForeverAmount(idx);
  closeModal();
  state.cash += amount;
  log(`💼 ${tr('Приложение продано инвесторам навсегда','App sold to investors for good')}: «${esc(site.name)}» → +${fmt(amount)}`);
  toast(`💼 ${tr('Продано навсегда','Sold for good')}: +${fmt(amount)}`);
  playSound('sell');
...
// app's current income, and ends the deal once its term is up or the
// app it was tied to is gone (sold/merged away).
...
function buildInvestorHtml(idx){
  ...
  const sellAmount = investorSellForeverAmount(idx);
  const dealAmount = investorDealAmount(idx);
  return `<div class="section-title">💼 ${tr('Инвестор','Investor')}</div>
    <div class="card glass" style="margin-bottom:10px;">
      <div class="card-title">🏳️ ${tr('Продать навсегда','Sell for good')}</div>
      <div class="card-sub">${tr('Приложение покидает портфель безвозвратно, одна крупная выплата','The app leaves your portfolio for good, one large payout')}</div>
      <div class="btn-row"><button class="btn btn-outline btn-block" onclick="confirmSellForever(${idx})">${tr('Продать за','Sell for')} ${fmt(sellAmount)}</button></div>
    </div>
    <div class="card glass">
      <div class="card-title">🤝 ${tr('Сделка с инвестором','Investor deal')}</div>
      <div class="card-sub">${tr(`Приложение остаётся у вас. Инвестор получает ${Math.round(INVESTOR_SHARE*100)}% дохода этого приложения в течение ${INVESTOR_TERM_DAYS} игровых дней, затем сделка сама заканчивается`,`The app stays yours. The investor takes ${Math.round(INVESTOR_SHARE*100)}% of this app's income for ${INVESTOR_TERM_DAYS} in-game days, then the deal ends on its own`)}</div>
      ...
      ${otherDealActive?`<div class="card-sub" style="color:var(--orange);margin-top:6px;">${tr('Уже есть активная сделка на другом приложении','A deal is already active on another app')}</div>`:''}
    </div>`;
}
function confirmSellForever(idx){
  const site = state.sites[idx];
  if(!site) return;
  const amount = investorSellForeverAmount(idx);
  openModal(`<h3>🏳️ ${tr('Продать','Sell')} «${esc(site.name)}» ${tr('инвесторам навсегда?','to investors for good?')}</h3>
    <p style="color:var(--dim);font-size:12.5px;margin-bottom:14px;">${tr('Разовая выплата','One-time payout')} <b>${fmt(amount)}</b>. ${tr('Приложение безвозвратно покинет портфель — это нельзя отменить.','The app leaves your portfolio for good — this cannot be undone.')}</p>
    <div class="btn-row"><button class="btn btn-outline btn-block" onclick="closeModal()">${S('Отмена')}</button><button class="btn btn-red btn-block" onclick="sellToInvestorForever(${idx})">${tr('Продать навсегда','Sell for good')}</button></div>`);
}
...
function confirmIpoSite(idx){
  ...
  if(renovationStage(site) < IPO_MIN_RENOVATIONS){ toast(tr(`Нужно минимум ${IPO_MIN_RENOVATIONS} обновления приложения для IPO`,`Needs at least ${IPO_MIN_RENOVATIONS} site renovations for an IPO`)); playSound('error'); return; }
  const value = ipoValue(idx);
  openModal(`<h3>📈 Вывести «${esc(site.name)}» на биржу?</h3>
    <p style="color:var(--dim);font-size:12.5px;margin-bottom:14px;">Разовая крупная выплата <b>${fmt(value)}</b> кэшем. Приложение остаётся у вас и продолжает работать, но его доход падает вдвое навсегда — акции теперь у публичных инвесторов.</p>
...
function ipoSite(idx){
  ...
  state.cash += value;
  site.ipoed = true;
  log(`📈 IPO «${esc(site.name)}» (${type.name}): разовая выплата ${fmt(value)}, доход приложения -50% навсегда`);
...
function buildHireModalHtml(idx){
  ...
  const cards = candidates.map(function(level){
    ...
    return `<div class="card glass" style="margin-bottom:10px;">
      <div class="card-row">
        ...
        <div style="flex:1">
          <div class="card-title">${tr(meta.name, meta.nameEn)}</div>
      ...
      <div class="btn-row"><button class="btn btn-violet btn-block" ${afford?'':'disabled'} onclick="confirmHireCandidate(${idx},${level})">${tr('Нанять за','Hire for')} ${fmt(cost)}</button></div>
    </div>`;
  }).join('');
  return `<h3>👥 ${tr('Выбор кандидата','Choose a candidate')}</h3>
    <p style="color:var(--dim);font-size:12.5px;margin-bottom:12px;">${tr('Более опытные кандидаты стоят дороже и просят бóльшую зарплату каждый месяц, но сильнее поднимают доход.','More experienced candidates cost more and draw a bigger monthly salary, but boost income harder.')}</p>
    <div id="hire-modal-body">${cards}</div>`;
}
function openHireModal(idx){ openModal(buildHireModalHtml(idx)); }
function confirmHireCandidate(idx, level){
  const site = state.sites[idx];
  if(!site) return;
  const cap = employeeCap(site);
  if(site.employees >= cap){ toast(tr('Нужна прокачка инфраструктуры для найма','Infrastructure upgrade needed to hire')); return; }
  ...
  bumpQuest('hire');
  const meta = empLevelMeta(level);
  log(`👤 ${tr('Нанят','Hired')} ${esc(tr(meta.name,meta.nameEn))} ${tr('в проект','for project')} ${esc(site.name)}`);
  playSound('buy');
  closeModal();
  renderAll(); save();
...
function updateGameClock(){
  ...
  el.textContent = `🕒 ${formatGameClockTime(gameSecondsOfDay)} · ${tr('до дня','until day')} ${state.day+1}: ${rh}${tr('ч','h')} ${rm}${tr('м','m')}`;
}
...
function checkBankruptcy(){
  ...
  if(state.cash < 0){
    ...
    if(state.difficulty==='hardcore' && !state.bankruptcy.rescueShown){
...
function resetAfterBankruptcy(){
  ...
  state.lastRankIndex = 0;
  state.activeEvents = [];
  state.loan = {principal:0, rating:0, takenDay:null, type:null, dueDay:null, lumpTotal:0};
  state.shorts = {};
  state.finance = {incomeHist:[],expenseHist:[],todayIncome:0,todayExpenses:0,dailyHistory:[],lastTickCash:state.cash};
  state.payroll = {owed:0, overdueDays:0, lastAssessDay:state.day, audited:false};
...
function buildLoanModalHtml(urgent){
  ...
  if(debt>0){
    ...
    ${ratingInfo}
    <div class="btn-row">
      <button class="btn btn-violet btn-block" ${avail<1?'disabled':''} onclick="takeLoan(${Math.round(avail*0.25)});closeModal();">${tr('Взять ещё 25%','Take another 25%')}</button>
  ...
  return `
    ...
    <div class="card glass" style="margin-bottom:10px;">
      ...
      <div class="btn-row">
        <button class="btn btn-violet btn-block" ${cap<1?'disabled':''} onclick="takeLoan(${Math.round(cap*0.25)});closeModal();">${tr('Взять 25%','Take 25%')} — ${fmt(Math.round(cap*0.25))}</button>
        <button class="btn btn-violet btn-block" ${cap<1?'disabled':''} onclick="takeLoan(${cap});closeModal();">${tr('Взять максимум','Take maximum')} — ${fmt(cap)}</button>
      </div>
    </div>
    <div class="card glass">
      ...
      <div class="btn-row"><button class="btn btn-outline btn-block" ${cap<1?'disabled':''} onclick="takeLumpsumLoan(${lumpAmount});closeModal();">${tr('Взять','Take')} ${fmt(lumpAmount)} → ${tr('вернуть','repay')} ${fmt(lumpOwed)}</button></div>
    </div>
...
function confirmReset(){
  openModal(`
    ...
    <p style="color:var(--dim);font-size:13px;margin-bottom:16px;">${tr('Это действие необратимо. Все приложения, акции, недвижимость и статус будут потеряны.','This action is irreversible. All sites, stocks, real estate, and status items will be lost.')}</p>
    <div class="btn-row">
...
function updateSiteViewLight(site, type, income){
  ...
  // Feature/patch dev progress bar — ticks every second while a dev cycle
  // is active (this was previously only redrawn on release, so the bar
  // looked frozen even though staff were filling it in the background).
  if(site.dev){
    const pct = Math.min(100, Math.round(site.dev.progress/site.dev.need*100));
    const fillEl = document.getElementById('sv-dev-progress-fill');
    if(fillEl) fillEl.style.width = pct+'%';
    const pctEl = document.getElementById('sv-dev-progress-pct');
...
function buildPageMockupHtml(idx){
  ...
  if(stage===1){
    return `<div class="sp-construction">
      <div class="big">🛠️</div>
      <div style="font-weight:700;color:#1a1d24;margin-bottom:6px;">Приложение в разработке</div>
  ...
  // App-store-style listing header (unique to Apps mode): icon tile + name +
  // star rating + category tag + install button, instead of a website navbar.
  // The rating is a deterministic function of design/traffic level so it
  // visibly climbs as the app matures, same spirit as the traffic bar-chart.
  const ratingValue = Math.min(5, 3.3 + stage*0.3 + Math.min(traffic,10)*0.03).toFixed(1);
  const fullStars = Math.round(parseFloat(ratingValue));
  const starsHtml = '★'.repeat(fullStars) + '☆'.repeat(5-fullStars);
  const header = `
    <div class="sp-header app-listing-header" style="--site-accent:${vis.accent}">
      <div class="app-icon-tile">${type.icon}</div>
      <div class="app-listing-meta">
        <div class="app-listing-name">${esc(site.name)}${verified?'<span class="sp-verified">✓</span>':''}</div>
        <div class="app-listing-sub"><span class="app-stars">${starsHtml}</span><span>${ratingValue}</span>${stage>=3?`<span>· ${vis.navLabel}</span>`:''}</div>
      </div>
      <button class="sp-cta app-install-btn">${vis.ctaLabel}</button>
    </div>`;
  const hero = `
    <div class="sp-hero">
      <h2>${vis.heroTitle}</h2>
...
function buildTracksHtml(idx){
  ...
  const capLevel = trackMaxLevel(site);
  const prio = priorityTrackId(type);
  const devActive = !!site.dev;
  let html = `<div class="card glass" style="margin-bottom:10px;padding:10px 14px;display:flex;align-items:center;gap:8px;">
    <span style="font-size:18px;">🎯</span>
    <div style="flex:1;font-size:12.5px;color:var(--dim);">${tr('Приоритет категории','Category priority')}: <b style="color:var(--text);">${priorityTrackLabel(type)}</b> — ${tr('сильно отставая по нему, приложение не выйдет в стабильный плюс','falling badly behind on it, the app will not reach stable profit')}</div>
  </div>`;
  if(devActive){
    const pct = Math.min(100, Math.round(site.dev.progress/site.dev.need*100));
    const isPatch = site.dev.kind==='patch';
    const label = isPatch ? tr('Разработка патча','Developing a patch')
      : `${tr('Разработка','Developing')}: ${TRACK_META[site.dev.trackId].icon} ${L(TRACK_META[site.dev.trackId],'name')} → ${tr('ур.','lvl')} ${site.tracks[site.dev.trackId]+site.dev.qty}`;
    html += `<div class="card glass" style="margin-bottom:10px;">
      <div class="card-title">🛠️ ${label}</div>
      <div class="card-sub">${tr('Штат ускоряет разработку — наймите больше сотрудников, чтобы релиз вышел быстрее','More staff speeds up development — hire more employees to ship sooner')} (👥 ${site.employees})</div>
      <div class="progress-bar" style="margin-top:8px;"><div id="sv-dev-progress-fill" style="width:${pct}%;background:var(--violet,#bf5af2);"></div></div>
      <div class="card-sub" id="sv-dev-progress-pct" style="margin-top:6px;text-align:right;">${pct}%</div>
    </div>`;
  } else if(site.bugged){
    ...
    html += `<div class="card glass" style="margin-bottom:10px;border-color:rgba(255,159,10,.4);">
      ...
      <div class="card-sub">${tr('Отзывы жалуются — доход снижен, пока не выпущен патч','Reviews are complaining — income is reduced until a patch ships')} (${sev==='major'?'-25%':'-12%'})</div>
      <div class="btn-row"><button class="btn btn-outline btn-block" ${state.cash<cost?'disabled':''} onclick="startPatch(${idx})">🔧 ${tr('Начать разработку патча','Start developing a patch')} — ${fmt(cost)}</button></div>
    </div>`;
  }
  html += TRACK_ORDER.map(key=>{
    ...
    if(atCap){
      return `<div class="card glass track-row">
        <div class="track-top">
          ...
          <div style="flex:1">
            <div class="track-name">${t.name}${star} <span class="track-lvl">Ур. ${lvl}/${capLevel}</span></div>
            <div class="track-desc">${t.desc}</div>
          </div>
        </div>
        <div class="track-maxplate">🔒 МАКС. УРОВЕНЬ — объедините приложение или переродитесь</div>
      </div>`;
    }
    ...
    const dis = devActive; // one dev cycle at a time — see the progress card above
    return `<div class="card glass track-row">
      <div class="track-top">
        <div class="track-icon" style="background:${t.color}22;color:${t.color};">${t.icon}</div>
        <div style="flex:1">
      ...
      <div class="qty-row">
        <button class="qty-btn aff-btn" data-aff-cost="${cost}" ${(dis||state.cash<cost)?'disabled':''} onclick="startFeatureDev(${idx},'${key}',1)"><span class="qn">×1</span><span class="qc">${fmt(cost)}</span></button>
        <button class="qty-btn aff-btn" data-aff-cost="${cost5}" ${(dis||state.cash<cost5||qty5<1)?'disabled':''} onclick="startFeatureDev(${idx},'${key}',${qty5})"><span class="qn">×${qty5}</span><span class="qc">${fmt(cost5)}</span></button>
        <button class="qty-btn aff-btn" data-aff-cost="${cost10}" ${(dis||state.cash<cost10||qty10<1)?'disabled':''} onclick="startFeatureDev(${idx},'${key}',${qty10})"><span class="qn">×${qty10}</span><span class="qc">${fmt(cost10)}</span></button>
        <button class="qty-btn qty-max aff-btn" data-aff-cost="${cost}" ${(dis||state.cash<cost)?'disabled':''} onclick="startFeatureDev(${idx},'${key}','max')"><span class="qn">МАКС${maxQty>1?' ×'+maxQty:''}</span><span class="qc">${maxQty>0?fmt(maxCost):'—'}</span></button>
      </div>
    </div>`;
  }).join('');
  return html;
...
function buildEmployeesCardHtml(idx){
  ...
  for(let i=0;i<cap;i++){
    ...
    const title = lv ? `${tr(empLevelMeta(lv).name, empLevelMeta(lv).nameEn)}` : '';
    empDots += `<div class="emp-dot ${i<site.employees?'filled':''}" title="${esc(title)}">${lv?empLevelMeta(lv).icon:''}</div>`;
  }
  ...
  return `
    <div class="card-row">
      ...
      <div style="flex:1">
        ...
        <div class="card-sub">Уровень нанятого определяет прибавку к доходу и зарплату. Зарплата ${fmt(monthly)}/мес спишется в общем платёжном дне. Больше слотов — прокачивайте инфраструктуру.</div>
        <div class="emp-dots">${empDots}</div>
      </div>
    </div>
    <div class="btn-row">
      <button class="btn btn-violet btn-block${site.employees<cap?' aff-btn':''}" ${site.employees<cap?`data-aff-cost="${empCost}"`:''} ${state.cash<empCost||site.employees>=cap?'disabled':''} onclick="hireForSite(${idx})">Нанять — от ${fmt(empCost)}</button>
    </div>`;
}
function buildTrafficHtml(idx){
  ...
  const tsrc = trafficBreakdown(site);
  return `
    <div class="section-title">Источники загрузок</div>
    <div class="card glass">
...
function buildMergeHtml(idx){
  ...
  const cost = mergeCost(idx);
  return `
    <div class="section-title">🔗 Слияние приложений</div>
    <div class="card glass" style="border-color:rgba(64,200,228,.35);background:rgba(64,200,228,.08);">
      ...
      <div class="card-sub">Оба приложения на макс. уровне трека — объедините их, чтобы поднять потолок на +${MERGE_CAP_BONUS} и продолжить расти.</div>
      <div class="btn-row"><button class="btn btn-cyan btn-block" ${state.cash<cost?'disabled':''} onclick="openMergeModal(${idx})">🔗 Объединить с другим приложениеом за ${fmt(cost)}</button></div>
    </div>`;
}
function buildRenovationHtml(idx){
  ...
  if(stage >= RENOVATION_MAX_STAGE){
    return `
      <div class="section-title">🛠️ ${tr('Обновление приложения','Site renovation')} · ${tr('этап','stage')} ${stage}/${RENOVATION_MAX_STAGE}</div>
      <div class="card glass" style="border-color:rgba(191,90,242,.35);background:rgba(191,90,242,.08);">
        ...
        <div class="card-sub">${tr('Это приложение обновлено по максимуму — теперь его можно вывести на IPO.','This site is fully renovated — it can now be taken public via IPO.')}</div>
      </div>`;
  }
  ...
  if(!maxed){
    ...
    statusLine = tr('Всё готово — можно обновлять приложение','Everything ready — you can renovate the site');
  }
  return `
    <div class="section-title">🛠️ ${tr('Обновление приложения','Site renovation')}${stage>0?` · ${tr('этап','stage')} ${stage}/${RENOVATION_MAX_STAGE}`:` · 0/${RENOVATION_MAX_STAGE}`}</div>
...
function buildInsuranceHtml(idx){
  ...
  if(site.insured){
    return `
      ...
      <div class="card glass" style="border-color:rgba(48,209,88,.3);background:rgba(48,209,88,.08);">
        <div class="card-title">🛡️ Приложение защищено</div>
  ...
  return `
    <div class="section-title">🛡️ Страхование</div>
    <div class="card glass">
      <div class="card-title">Застраховать приложение</div>
      ...
      <div class="card glass">
        <div class="card-row">
          ...
          <div style="flex:1">
            ...
            <div class="card-sub">Разовая покупка на всю игру — приложения с включённым авто-менеджером сами нанимают сотрудников и покупают дешёвые апгрейды</div>
          </div>
        </div>
  ...
  return `
    ...
    <div class="card glass">
      <div class="switch-row" style="border-bottom:none;">
        <span>Авто-найм и авто-прокачка на этом приложении</span>
  ...
  if(site.ipoed){
    return `
      ...
      <div class="card glass" style="border-color:rgba(10,132,255,.3);background:rgba(10,132,255,.08);">
        ...
        <div class="card-sub">Уже выведена на биржу — доход приложения снижен вдвое навсегда, разовая выплата уже получена</div>
      </div>`;
  }
  const stage = renovationStage(site);
      ...
      <div class="card glass">
        <div class="card-row">
          ...
          <div style="flex:1">
            ...
            <div class="card-sub">${tr(`Нужно минимум ${IPO_MIN_RENOVATIONS} обновления приложения (сейчас ${stage}/${IPO_MIN_RENOVATIONS}) — прокачайте ветки до максимума и обновите приложение через раздел «Обновление приложения»`,`Needs at least ${IPO_MIN_RENOVATIONS} site renovations (currently ${stage}/${IPO_MIN_RENOVATIONS}) — max out the tracks and renovate the site via the "Site renovation" section`)}</div>
          </div>
        </div>
      </div>`;
  }
    ...
    <div class="card glass">
      <div class="card-row">
        ...
        <div style="flex:1">
          ...
          <div class="card-sub">Разовая крупная выплата — но доход приложения упадёт вдвое навсегда. Трейд-офф вместо продажи: приложение остаётся у вас</div>
        </div>
      </div>
...
function buildSynergyHtml(idx){
  ...
  return `
    ...
    <div class="card glass" style="border-color:${good?'rgba(48,209,88,.3)':'rgba(255,159,10,.3)'};background:${good?'rgba(48,209,88,.08)':'rgba(255,159,10,.08)'};">
      <div class="card-title">${good?'+':''}${pct}% к доходу этого приложения</div>
      <div class="card-sub">${good?`Второе приложение того же типа даёт кросс-промо буст`:`${count} приложений этого типа насыщают рынок — доход каждого просел`}</div>
    </div>`;
}
...
function buildSiteView(idx){
  ...
  return `
    <div class="sv-top">
      ...
      <button class="sv-close" id="sv-device-btn" title="Переключить вид: телефон/планшет" onclick="toggleSitePreviewDevice()">${sitePreviewDesktop?'📲':'📱'}</button>
      <span class="sv-stage-pill" id="sv-stage-pill">${buildStagePillHtml(idx)}</span>
    </div>
    <div class="sv-panel-body">
      <div class="browser-frame app-device-frame${sitePreviewDesktop?' desktop-mode':''}" id="sv-browser-frame">
        <div class="app-notch"></div>
        <div class="app-status-bar">
          <span class="asb-time">9:41</span>
          <div class="asb-icons"><span>📶</span><span>🛜</span><span>🔋</span></div>
        </div>
      ...
      <div class="stat-strip" style="grid-template-columns:1fr 1fr 1fr;margin:0 0 6px;">
        <div class="stat-box glass"><div class="lbl">Доход приложения</div><div class="val num c-green"><span id="sv-income-val">${fmt(income)}/с</span><span id="sv-boost-badge" class="boost-badge" style="${boostActive?'':'display:none;'}">📢 x1.5 · ${boostSecsLeft}с</span></div></div>
      ...
      <div class="card glass">
        <div class="card-row">
          ...
          <div style="flex:1">
            ...
            <div class="card-sub">+50% к доходу приложения на 60 секунд</div>
          </div>
        </div>
        <div class="btn-row">
      ...
      <div id="sv-advanced-wrap" style="${sviteAdvancedOpen?'':'display:none;'}">
        ...
        <div id="sv-analyst">${buildAnalystHtml(idx)}</div>
      ...
      <div class="card glass" style="border-color:rgba(255,69,58,.25);">
        <div class="card-row">
          <div class="card-icon">💀</div>
          <div style="flex:1">
            <div class="card-title">Продать / закрыть приложение</div>
...
function confirmSellSite(idx){
  ...
  openModal(`<h3>💀 Продать «${esc(site.name)}»?</h3>
    <p style="color:var(--dim);font-size:12.5px;margin-bottom:14px;">Приложение и всё, что в него вложено, исчезнут безвозвратно. Слот освободится, вы получите <b>${fmt(value)}</b> кэшем.</p>
...
function sellSite(idx){
  ...
  const value = siteSellValue(idx);
  state.cash += value;
  log(`💀 ${tr('Продан приложение','Sold site')} «${esc(site.name)}» (${L(type,'name')}) ${tr('за','for')} ${fmt(value)}`);
...
function refreshSiteViewSections(idx, sections){
  ...
  if(sections.includes('tracks')){ set('sv-tracks', buildTracksHtml(idx)); set('sv-renovation', buildRenovationHtml(idx)); set('sv-merge', buildMergeHtml(idx)); set('sv-hybrid', buildHybridHtml(idx)); set('sv-synergy', buildSynergyHtml(idx)); set('sv-ads', buildAdsHtml(idx)); if(!state.sites[idx].ipoed) set('sv-ipo', buildIpoHtml(idx)); }
  ...
  if(sections.includes('analyst')) set('sv-analyst', buildAnalystHtml(idx));
...
function renderNextUnlockCard(nw){
  ...
  const locked = ALL_BUSINESS_TYPES
  ...
  el.style.display = '';
  const prevThreshold = ALL_BUSINESS_TYPES.filter(t=>t.unlockNetWorth <= nw).reduce((m,t)=>Math.max(m,t.unlockNetWorth),0);
...
function renderLog(){
  ...
  if(!state.log.length){ el.innerHTML = `<div class="empty">${tr('Пока тихо. Начните с покупки первого приложения.','It is quiet for now. Start by buying your first site.')}</div>`; return; }
...
function renderTypeCard(type){
  ...
  else if(!locked && !slotsFull && needsRenovation){ btnLabel = `🔒 ${tr(`Сначала обновите приложения до эт. ${RENOVATION_MAX_STAGE}`, `First renovate your sites to stage ${RENOVATION_MAX_STAGE}`)}`; disabled = true; }
...
function renderSites(){
  ...
  let html = `<div class="card glass" style="margin-bottom:16px;">
    <div class="card-title">🧩 ${tr('Слоты приложений','Site slots')}: ${state.sites.length}/${slots}</div>
  ...
  if(state.sites.length>0 && !allOwnedSitesFullyRenovated()){
    ...
    html += `<div class="card glass" style="margin-bottom:16px;border-color:rgba(255,159,10,.35);background:rgba(255,159,10,.07);">
      <div class="card-title">🏗️ ${tr('Новое приложение пока закрыто','New site locked for now')}</div>
      <div class="card-sub">${tr('Прокачайте все ветки до потолка и обновите текущие приложения до максимального этапа','Max out every track and renovate your current sites to the final stage')} (${worst}/${RENOVATION_MAX_STAGE}) ${tr('— тогда откроется покупка следующего','— then the next purchase unlocks')}.</div>
    </div>`;
  }
  ...
  if(!state.sites.length){
    html += `<div class="empty">🌱 ${tr('У вас пока нет приложений — купите первый выше','You do not have any sites yet — buy your first one above')}</div>`;
  } else {
    state.sites.forEach((site, idx)=>{
      const type = ALL_BUSINESS_TYPES.find(t=>t.id===site.typeId);
      const vis = SITE_VISUAL[type.id];
      ...
      html += `<div class="card glass" style="cursor:pointer;${state.boosty.unlocked?'border:1px solid rgba(255,214,10,.35);box-shadow:0 8px 30px rgba(0,0,0,.35),inset 0 1px 0 rgba(255,214,10,.1);':''}" onclick="openSiteView(${idx})">
        ...
        <div class="btn-row"><button class="btn btn-outline btn-block" onclick="event.stopPropagation();openSiteView(${idx})">${tr('Открыть приложение и прокачать →','Open site and upgrade →')}</button></div>
      </div>`;
    });
  }
  el.innerHTML = html;
  ...
  if(state.boosty.unlocked){
    ...
    el.innerHTML = `
      ...
      <ul style="margin:6px 0 10px 18px;color:var(--dim);font-size:12px;line-height:1.7;">
        ...
        <li>🤖 ${tr('Авто-менеджер приложений открыт бесплатно','Site auto-manager unlocked for free')}</li>
        <li>🏦 ${tr('Ставка по кредиту ниже на 20%, лимит выше на 25%','Loan rate 20% lower, limit 25% higher')}</li>
        <li>🧩 ${tr('+1 дополнительный слот под приложение','+1 extra site slot')}</li>
    ...
    el.innerHTML = `
      ...
      <ul style="margin:6px 0 10px 18px;color:var(--dim);font-size:12px;line-height:1.7;">
        ...
        <li>🤖 ${tr('Авто-менеджер приложений бесплатно','Site auto-manager for free')}</li>
        <li>🏦 ${tr('Льготная ставка и лимит по кредиту','Discounted loan rate and limit')}</li>
        <li>🧩 ${tr('+1 слот под приложение','+1 site slot')}</li>
...
function redeemBoosty(){
  ...
  log(tr('🚀 Активирована Boosty-подписка — безлимитный офлайн-доход, авто-менеджер, льготный кредит, +1 слот, бейдж CEO и ранний доступ к сезонным событиям','🚀 Boosty subscription activated — unlimited offline income, auto-manager, discounted loan, +1 slot, CEO badge and early access to seasonal events'));
  renderSettings();
  renderAll();
  save();
}
...
// This file no longer owns a pre-game main menu or mode switcher — both now
// live solely in the standalone index.html hub, the one place you pick
// Sites vs Apps. Loading this file always drops you straight into your
// save (or into new-game setup if there isn't one yet).
function enterGame(){
  if(hasSavedProgress()){
    nav('dash');
...
function confirmSetup(){
  ...
  // taxes/payroll start being assessed — so the first few minutes are all
  // momentum, not admin.
...
const ONBOARD_STEPS = [
  ...
  {icon:'🌐', title:'Начните с вкладки «Приложения»', titleEn:'Start with the "Sites" tab', text:'Внизу экрана откройте «Приложения» и купите свой первый бизнес — это основной источник дохода.', textEn:'At the bottom of the screen, open "Sites" and buy your first business — it\'s your main source of income.'},
  {icon:'📐', title:'Прокачивайте треки', titleEn:'Upgrade your tracks', text:'Внутри каждого приложения есть 3 трека: дизайн, загрузки, серверы. Прокачивайте самый дешёвый — это быстрее всего окупается. А деньги за рекламу зарабатывайте отдельно — в разделе «Реклама».', textEn:'Every site has 3 tracks: design, traffic, infrastructure. Upgrade the cheapest one — it pays off the fastest. Ad revenue is earned separately, in the "Ads" section.'},
  {icon:'💵', title:'Следите за чистыми активами', titleEn:'Keep an eye on net worth', text:'Карточка «Чистые активы» на дашборде — главный показатель роста. От неё зависят новые слоты под приложения и ранги.', textEn:'The "Net worth" card on the dashboard is your main growth metric. It determines new site slots and ranks.'},
...
function buildShareCardCanvas(){
  ...
  const stats = [
    ...
    [tr('Приложений','Sites'), state.sites.length],
...
function runCompare(){
  ...
  const rows = [
    ...
    ['Приложений', me.s, friend.s, me.s>=friend.s],
...
const I18N = {
  ru: {
    'nav.dash':'Дашборд', 'nav.sites':'Приложения', 'nav.market':'Биржа', 'nav.estate':'Активы', 'nav.inbox':'Уведомления', 'nav.settings':'Ещё',
    ...
    'dash.taxes':'Налоги',
    'dash.quickactions':'Быстрые действия','dash.upgradebiz':'Прокачать бизнес','dash.upgradebizsub':'Выберите, что именно улучшить','dash.gotosites':'Перейти к приложениям',
    ...
    'sites.sub':'Прокачивайте дизайн, загрузки и серверы — а рекламу размещайте отдельно ради быстрого кэша',
    ...
    'settings.modes.title':'Режимы игры скоро появятся','settings.modes.sub':'Приложения, приложения и нейросети как отдельные режимы — в разработке',
